...
- **Contact information**: Document contact info for all relevant team members.
- **Notification procedures**: This answers the "Who/What/When/Why/How" of notifying relevant members.
...
Most disaster recovery playbooks define the goals and allowed impact of a given failure using two common terms: **Recovery Time Objective** and **Recovery Point Objective**.
...
{% asset '{{ page.asset-path }}'/rto-rpo-example-wikipedia.png alt='RTO & RPO Example - Courtesy of Wikipedia' %}{: .align-center }
...
Ensure that all [Prerequisites][#prerequisites] have been met.  All playbooks, dependency definitions, and other relevant documentation should placed in a singular, globally accessible location so every single team member has immediate access to that information.  Maintaining a single repository for the information also maintains consistency across the team, so there's never any confusion about the steps in a particular scenario or what is defined as a critical dependency.
...
The last step necessary to complete **Resiliency Stage 1**
